...
Tracking::Tracking(int frame_rate, int track_buffer,int n_init)
{
	track_thresh = 0.5;
	high_thresh = 0.6;
	match_thresh = 0.8;
...
vector<STrack> Tracking::update(const vector<Object>& objects)
{
	...
	if (objects.size() > 0)
	{
		for (int i = 0; i < objects.size(); i++)
		{
			...
			STrack strack(STrack::tlbr_to_tlwh(tlbr_), score,this->n_init,objects[i].key_points);
	...
	for (int i = 0; i < removed_stracks.size(); i++)
	{
		...
		{
		 	vector<STrack>::iterator iter=this->removed_stracks.begin();
		 	this->removed_stracks.erase(iter);
		}
