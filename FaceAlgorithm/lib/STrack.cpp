...
STrack::STrack(vector<float> tlwh_, float score,int n_init,std::vector<cv::Point2f>key_points)
{
	_tlwh.resize(4);
...
void STrack::re_activate(STrack &new_track, int frame_id, bool new_id)
{
	...
	this->tracklet_len = 0;
	this->state = TrackState::Tracked;
	this->key_points=new_track.key_points;
	this->is_activated = true;
...
void STrack::update(STrack &new_track, int frame_id)
{
	...
	this->state = TrackState::Tracked;
	if (this->tracklet_len> this->n_init)
	{
		this->is_activated = true;
	}
	n_frame++;
